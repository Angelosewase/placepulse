    ...
    return (
      <div className="w-full px-4 py-2 cursor-pointer relative rounded-lg">
        ...
        <div className="w-full h-full px-4 pb-6 flex flex-col items-start justify-end absolute top-0 left-0 z-20 bg-[#36353518] rounded-lg">
          ...
          <button className="w-full py-3 mt-3 rounded-md flex items-center font-medium justify-center bg-[#699BFE] text-white" onClick={()=> navigate(`/booking/place/${data.id}`)}>Book a {capitalize(data.type)}</button>
        </div>
      </div>
    );
  };
