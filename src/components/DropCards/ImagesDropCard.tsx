...
const ImagesDropCard = ({ minHeight, buttonPadding, textSize, selectedImage, setSelectedImage }: { minHeight: string | number; buttonPadding?: number; textSize?: string; selectedImage: File | null; setSelectedImage: any; }) => {
      ...
      <div className={`w-full h-[${minHeight}] cursor-pointer relative`}>
        {selectedImage ? (
          <div className='w-full h-full relative'>
            <img src={URL.createObjectURL(selectedImage)} alt="Selected" className='w-full h-full object-cover rounded-lg cursor-pointer' />
            {/* close image */}
            <button className='absolute top-3 right-3 bg-neutral-200 rounded-full p-2' onClick={handleDeselect}>
              <IoClose />
            </button>
          </div>
        ) : (
